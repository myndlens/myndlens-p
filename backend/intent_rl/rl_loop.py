"""Intent RL Multi-Iteration Runner — runs N iterations with reinforcement feedback.

After each iteration:
  1. Collect failures
  2. Store correction facts in Digital Self (so memory recall picks them up next run)
  3. Run again — LLM now gets correction context from memory
  4. Track accuracy progression across iterations
"""
import asyncio
import logging
from datetime import datetime, timezone
from typing import Optional

from intent_rl.dataset_v2 import INTENT_DATASET_V2
from intent_rl.seed_digital_self import RL_USER_ID

logger = logging.getLogger(__name__)


# ── State ────────────────────────────────────────────────────────────────────

_rl_loop_state = {
    "running": False,
    "current_iteration": 0,
    "total_iterations": 0,
    "iterations": [],   # list of {iteration, intent_accuracy, sub_coverage, entity_coverage, failures, corrections_added}
    "started_at": None,
    "completed_at": None,
}


def get_rl_loop_state() -> dict:
    return _rl_loop_state


async def run_rl_loop(n_iterations: int = 10, delay_between_calls: float = 0.3) -> str:
    """Run N iterations of the RL loop with feedback injection."""
    global _rl_loop_state

    if _rl_loop_state["running"]:
        return "already_running"

    _rl_loop_state = {
        "running": True,
        "current_iteration": 0,
        "total_iterations": n_iterations,
        "iterations": [],
        "started_at": datetime.now(timezone.utc).isoformat(),
        "completed_at": None,
    }

    asyncio.create_task(_execute_rl_loop(n_iterations, delay_between_calls))
    return "started"


async def _execute_rl_loop(n_iterations: int, delay: float) -> None:
    """Execute the full RL loop."""
    global _rl_loop_state

    from l1.scout import run_l1_scout
    from memory.retriever import store_fact
    from intent_rl.seed_digital_self import seed_digital_self, clear_digital_self
    from intent_rl.runner_v2 import (
        _check_intent_match, _check_sub_intents, _check_entity_resolution,
        _INTENT_KEYWORDS,
    )
    from core.database import get_db

    dataset = INTENT_DATASET_V2

    for iteration in range(1, n_iterations + 1):
        _rl_loop_state["current_iteration"] = iteration
        logger.info("[RL Loop] Starting iteration %d/%d", iteration, n_iterations)

        # ── Run all cases ──
        results = []
        for case in dataset:
            session_id = f"rl_loop_i{iteration}_{case['id']}"
            try:
                draft = await run_l1_scout(
                    session_id=session_id,
                    user_id=RL_USER_ID,
                    transcript=case["broken_thoughts"],
                )

                all_hyps = []
                for h in draft.hypotheses:
                    all_hyps.append({
                        "hypothesis": h.hypothesis,
                        "action_class": h.action_class,
                        "confidence": h.confidence,
                        "dimensions": h.dimension_suggestions,
                    })

                top = draft.hypotheses[0] if draft.hypotheses else None
                extracted_hyp = top.hypothesis if top else ""
                extracted_class = top.action_class if top else "NONE"

                intent_match = _check_intent_match(
                    extracted_hyp, extracted_class, all_hyps, case["main_intent"]
                )
                sub_found, sub_cov = _check_sub_intents(
                    extracted_hyp, all_hyps, case["sub_intents"]
                )
                ent_resolved, ent_cov = _check_entity_resolution(
                    extracted_hyp, all_hyps, case.get("expected_entities", [])
                )

                results.append({
                    "case_id": case["id"],
                    "main_intent": case["main_intent"],
                    "intent_match": intent_match,
                    "sub_coverage": sub_cov,
                    "entity_coverage": ent_cov,
                    "extracted_hypothesis": extracted_hyp,
                    "extracted_class": extracted_class,
                    "broken_thoughts": case["broken_thoughts"],
                })

            except Exception as e:
                logger.error("[RL Loop] Case %d failed: %s", case["id"], e)
                results.append({
                    "case_id": case["id"],
                    "main_intent": case["main_intent"],
                    "intent_match": False,
                    "sub_coverage": 0.0,
                    "entity_coverage": 0.0,
                    "extracted_hypothesis": f"ERROR: {e}",
                    "extracted_class": "ERROR",
                    "broken_thoughts": case["broken_thoughts"],
                })

            # Tight delay between calls
            await asyncio.sleep(delay)

        # ── Score iteration ──
        correct = sum(1 for r in results if r["intent_match"])
        total = len(results)
        intent_acc = correct / total if total else 0
        avg_sub = sum(r["sub_coverage"] for r in results) / total if total else 0
        avg_ent = sum(r["entity_coverage"] for r in results) / total if total else 0
        failures = [r for r in results if not r["intent_match"]]

        # ── Inject corrections into Digital Self ──
        corrections_added = 0
        for f in failures:
            correction_text = (
                f"INTENT CORRECTION: When user says something like '{f['broken_thoughts'][:80]}...' "
                f"the main intent is '{f['main_intent']}', not '{f['extracted_class']}'. "
                f"Keywords for {f['main_intent']}: {', '.join(_INTENT_KEYWORDS.get(f['main_intent'], [])[:5])}"
            )
            try:
                await store_fact(
                    user_id=RL_USER_ID,
                    text=correction_text,
                    fact_type="CORRECTION",
                    provenance="RL_FEEDBACK",
                    metadata={
                        "iteration": iteration,
                        "case_id": f["case_id"],
                        "expected_intent": f["main_intent"],
                        "got_class": f["extracted_class"],
                    },
                )
                corrections_added += 1
            except Exception as e:
                logger.warning("[RL Loop] Failed to store correction: %s", e)

        # ── Store iteration results ──
        iter_result = {
            "iteration": iteration,
            "intent_accuracy": round(intent_acc * 100, 1),
            "sub_intent_coverage": round(avg_sub * 100, 1),
            "entity_coverage": round(avg_ent * 100, 1),
            "correct": correct,
            "total": total,
            "failure_count": len(failures),
            "corrections_added": corrections_added,
            "failures": [
                {
                    "case_id": f["case_id"],
                    "main_intent": f["main_intent"],
                    "got": f["extracted_class"],
                    "hypothesis": f["extracted_hypothesis"][:80],
                }
                for f in failures
            ],
        }
        _rl_loop_state["iterations"].append(iter_result)

        logger.info(
            "[RL Loop] Iteration %d/%d: accuracy=%.1f%% sub=%.1f%% ent=%.1f%% failures=%d corrections=%d",
            iteration, n_iterations, intent_acc * 100, avg_sub * 100, avg_ent * 100,
            len(failures), corrections_added,
        )

        # Persist to MongoDB after each iteration
        db = get_db()
        await db.intent_rl_loop.replace_one(
            {"loop_id": _rl_loop_state["started_at"]},
            {
                "loop_id": _rl_loop_state["started_at"],
                "current_iteration": iteration,
                "total_iterations": n_iterations,
                "iterations": _rl_loop_state["iterations"],
                "updated_at": datetime.now(timezone.utc).isoformat(),
            },
            upsert=True,
        )

    # ── Loop complete ──
    _rl_loop_state["running"] = False
    _rl_loop_state["completed_at"] = datetime.now(timezone.utc).isoformat()

    # Final summary
    accs = [i["intent_accuracy"] for i in _rl_loop_state["iterations"]]
    logger.info(
        "[RL Loop] COMPLETE. Accuracy progression: %s",
        " → ".join(f"{a}%" for a in accs),
    )
