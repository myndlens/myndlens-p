...
async def api_start_intent_rl(batch_size: int = 100):
    """Start the intent RL test batch (100 cases, real Gemini calls).
    
    Runs in background. Poll /api/intent-rl/status for progress.
    """
    ...
    if current and current.in_progress:
        return {
            ...
            "progress": f"{current.completed}/{current.total}",
        }
...
async def api_intent_rl_status():
    ...
    run = get_current_run()
    if not run:
        return {"status": "no_run", "message": "No RL run in progress or completed. POST /api/intent-rl/run to start."}
...
async def api_intent_rl_history(limit: int = 10):
    """Get historical RL run results for comparison."""
