...
async def determine_skills(
    ...
    task = (
        ...
        "For EACH action in the mandate, determine HOW to execute it:\n"
        "1. EXISTING skill from the library handles it directly → use it\n"
        "2. Existing skill needs ADAPTATION → adapt it\n"
        "3. A DIRECT API exists (airline API, hotel API, etc.) → use API skill\n"
        "4. NO API exists → use BROWSER skills (Agent Browser, Web Scraper) to:\n"
        "   navigate to the website, search, fill forms, compare options, and complete the action.\n"
        "   This is how a human secretary would do it — go to qantas.com, search flights, book.\n"
        "5. Needs RESEARCH first → use web search skills, then execute\n\n"
        "IMPORTANT: Browser scraping is ALWAYS available as a fallback.\n"
        "If no direct API integration exists for an action, the execution path is:\n"
        "  Agent Browser → navigate to relevant website → fill in dimensions → execute.\n"
        "This applies to flight booking, hotel reservation, car rental, restaurant booking, etc.\n\n"
        "For each skill, specify the execution_method:\n"
        "  'api' — direct API call\n"
        "  'browser' — navigate website, fill forms, complete action\n"
        "  'hybrid' — API for search, browser for booking\n"
        "  'manual' — requires human handoff\n\n"
        "Output JSON:\n"
        "{\n"
        "  \"skill_plan\": [{\n"
        ...
        "    \"execution_method\": \"api\" | \"browser\" | \"hybrid\" | \"manual\",\n"
        "    \"target_url\": str (website to navigate if browser method),\n"
        "    \"reason\": str (why this skill and method),\n"
